"""
Backfill classifier predictions and Mahalanobis distances into
master_carvings.csv, and write a machine-readable data dictionary
documenting every column in the master datasets with its source
and derivation.
"""
...
# =============================================================
# Train the definitive axe-vs-mushroom classifier on the paper's
# own labeled corpus (Bevan All data)
# =============================================================
allf = pd.read_excel(BEVAN, sheet_name="All data")
FEATS = ["Circularity", "Aspect Ratio", "Roundness"]
XL_TO_CANON = {"Circ.": "Circularity", "AR": "Aspect Ratio", "Round": "Roundness"}

train_axes = allf[allf["Type"] == "Axe"][["Circ.", "AR", "Round"]].dropna()
train_axes.columns = FEATS
train_mus = allf[allf["Type"] == "Mushroom"][["Circ.", "AR", "Round"]].dropna()
train_mus.columns = FEATS
...
# Centroids for Mahalanobis
axe_std = scaler.transform(train_axes.values)
mus_std = scaler.transform(train_mus.values)
axe_mean = axe_std.mean(axis=0)
mus_mean = mus_std.mean(axis=0)
axe_cov_inv = np.linalg.pinv(np.cov(axe_std.T))
mus_cov_inv = np.linalg.pinv(np.cov(mus_std.T))

# =============================================================
# Backfill master_carvings
# =============================================================
carv = pd.read_csv(MASTER / "master_carvings.csv")
print(f"Loaded {len(carv)} carving rows.")

# Compute predictions for each row that has all 3 features
def predict_row(row):
    try:
        x = np.array([row["circularity"], row["aspect_ratio"], row["roundness"]])
        if np.isnan(x).any():
        ...
        d_axe = float(mahalanobis(xs, axe_mean, axe_cov_inv))
        d_mus = float(mahalanobis(xs, mus_mean, mus_cov_inv))
        closer = "mushroom" if d_mus < d_axe else "axe"
        return pd.Series([lda_p, rf_p, d_axe, d_mus, closer])
    except Exception:
        return pd.Series([None]*5)

new_cols = carv.apply(predict_row, axis=1)
new_cols.columns = ["lda_p_mushroom", "rf_p_mushroom",
                     "mahal_dist_to_axe_centroid",
                     "mahal_dist_to_mushroom_centroid",
                     "nearest_centroid"]

# Overwrite any existing prediction columns (from Ri Cruin summary) with the
# consistent ones we just computed
for c in new_cols.columns:
    carv[c] = new_cols[c]
# Drop the old Ri-Cruin-only columns that had different names
for old in ["mahal_d_axe", "mahal_d_mushroom", "closer_to"]:
    if old in carv.columns:
        carv = carv.drop(columns=[old])

# Overwrite the file
carv.to_csv(MASTER / "master_carvings.csv", index=False)

# Report
predicted = carv.dropna(subset=["nearest_centroid"])
print(f"Predictions attached to {len(predicted)}/{len(carv)} rows")
print("\nBy stone:")
for st, sub in predicted.groupby("stone"):
    lda_mus = (sub["lda_p_mushroom"] > 0.5).sum()
    rf_mus = (sub["rf_p_mushroom"] > 0.5).sum()
    mahal_mus = (sub["nearest_centroid"] == "mushroom").sum()
    print(f"  {st}  (n={len(sub)}):  LDA mus {lda_mus},  RF mus {rf_mus},  Mahal mus {mahal_mus}")

# =============================================================
# Data dictionary
# =============================================================
DICT = ROOT / "data" / "master" / "data_dictionary.md"

dictionary_md = """# Data dictionary
...
DICT.write_text(dictionary_md)
print(f"\nWrote {DICT}")

# Also update manifest to include the dictionary + backfilled columns
manifest_path = MASTER / "manifest.json"
manifest = json.load(open(manifest_path))
manifest["datasets"]["master_carvings.csv"]["backfilled_columns"] = [
    "lda_p_mushroom", "rf_p_mushroom",
    "mahal_dist_to_axe_centroid", "mahal_dist_to_mushroom_centroid",
    "nearest_centroid",
]
manifest["datasets"]["master_carvings.csv"]["n_rows"] = len(carv)
manifest["dictionary"] = "data_dictionary.md"
